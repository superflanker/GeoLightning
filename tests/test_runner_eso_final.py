...
from GeoLightning.Runners.RunnerESO import runner_ESO_process


def test_runner_ESO_final(fake_test=False):
    ...
    for k in [x * processes for x in range(0, num_groups)]:
        ...
        with ProcessPoolExecutor() as executor:
            temp_resultados = list(executor.map(runner_ESO_process, params))
    ...
    if fake_test:
        ...
        """
        paper_data = np.array(paper_data)
        output_file = os.path.join(basedir, "ESO_final_results.npy")
