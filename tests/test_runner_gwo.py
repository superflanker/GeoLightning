...
from GeoLightning.Runners.RunnerGWO import runner_GWO_process
...
def test_runner_GWO(fake_test=False):
    ...
    for k in [x * processes for x in range(0, num_groups)]:
        ...
        with ProcessPoolExecutor() as executor:
            temp_resultados = list(executor.map(runner_GWO_process, params))
