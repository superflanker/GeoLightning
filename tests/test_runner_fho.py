...
from GeoLightning.Runners.RunnerFHO import runner_FHO_process
...
def test_runner_FHO(fake_test=False):
    ...
    for k in [x * processes for x in range(0, num_groups)]:
        ...
        with ProcessPoolExecutor() as executor:
            temp_resultados = list(executor.map(runner_FHO_process, params))
    ...
    if fake_test:
        ...
        output_file = os.path.join(
            basedir, "FHO_deltas_distancia_refinado.npy")
        np.save(output_file, deltas_d)
        print(
        ...
        output_file = os.path.join(
            basedir, "FHO_centroides_espaciais_irls.npy")
        ...
        output_file = os.path.join(
            basedir, "FHO_centroides_temporais_irls.npy")
