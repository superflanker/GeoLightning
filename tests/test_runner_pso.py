...
from GeoLightning.Runners.RunnerPSO import runner_PSO_process
...
def test_runner_PSO(fake_test=False):
    ...
    for k in [x * processes for x in range(0, num_groups)]:
        ...
        with ProcessPoolExecutor() as executor:
            temp_resultados = list(executor.map(runner_PSO_process, params))
    ...
    if fake_test:
        ...
        output_file = os.path.join(
            basedir, "PSO_deltas_distancia_refinado.npy")
        np.save(output_file, deltas_d)
        print(
        ...
        output_file = os.path.join(
            basedir, "PSO_centroides_espaciais_irls.npy")
        ...
        output_file = os.path.join(
            basedir, "PSO_centroides_temporais_irls.npy")
