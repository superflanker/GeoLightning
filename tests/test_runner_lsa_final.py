...
def test_runner_LSA(fake_test=False):
    ...
    for k in [x * processes for x in range(0, num_groups)]:
        ...
        with ProcessPoolExecutor() as executor:
            temp_resultados = list(executor.map(runner_LSA_process, params))
    ...
    if fake_test:
        ...
        """
        paper_data = np.array(paper_data)
        output_file = os.path.join(basedir, "LSA_final_results.npy")
