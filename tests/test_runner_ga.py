...
from GeoLightning.Runners.RunnerGA import runner_GA_process
...
def test_runner_GA(fake_test=False):
    ...
    for k in [x * processes for x in range(0, num_groups)]:
        ...
        with ProcessPoolExecutor() as executor:
            temp_resultados = list(executor.map(runner_GA_process, params))
    ...
    if fake_test:
        ...
        output_file = os.path.join(
            basedir, "GA_deltas_distancia_refinado.npy")
        np.save(output_file, deltas_d)
        print(
        ...
        output_file = os.path.join(
            basedir, "GA_centroides_espaciais_irls.npy")
        ...
        output_file = os.path.join(
            basedir, "GA_centroides_temporais_irls.npy")
