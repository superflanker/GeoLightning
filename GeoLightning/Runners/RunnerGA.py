"""
StelaGA Runner Wrapper
=======================

Summary
-------
Wrapper function for executing the StelaGA algorithm in the context of
...
- Execution of the StelaGA algorithm
...
from GeoLightning.Solvers.StelaGA import StelaGA
from GeoLightning.Utils.Constants import *


def runner_GA(event_positions: np.ndarray,
    ...
    """
    Executes the Genetic Algorithm (GA) algorithm for estimating 
    the origin positions of events based on clustered detections and arrival time data.

    This function applies the GA metaheuristic to solve the multilateration problem 
    ...
    max_epochs : int, optional
        Maximum number of iterations (epochs) for the GA algorithm. Default is 100.

    max_population : int, optional
        Number of candidate solutions in the GA population. Default is 100.
    ...
    tuple
        ...
        sol_best_fitness : np.ndarray  
            Best fitness value obtained by GA for each cluster.
    ...
    solver = StelaGA(epoch=max_epochs,
...
def runner_GA_process(params):
    ...
    return runner_GA(event_positions=event_positions,
                     event_times=event_times,
                     spatial_clusters=spatial_clusters,
                     sensor_tt=sensor_tt,
                     sensor_indexes=sensor_indexes,
