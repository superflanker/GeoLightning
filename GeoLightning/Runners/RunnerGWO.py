"""
StelaGWO Runner Wrapper
=======================

Summary
-------
Wrapper function for executing the StelaGWO algorithm in the context of
...
- Execution of the StelaGWO algorithm
...
from GeoLightning.Solvers.StelaGWO import StelaGWO
from GeoLightning.Utils.Constants import *


def runner_GWO(event_positions: np.ndarray,
    ...
    """
    Executes the Grey Wolf Optimizer (GWO) algorithm for estimating 
    the origin positions of events based on clustered detections and arrival time data.

    This function applies the GWO metaheuristic to solve the multilateration problem 
    ...
    max_epochs : int, optional
        Maximum number of iterations (epochs) for the GWO algorithm. Default is 100.

    max_population : int, optional
        Number of candidate solutions in the GWO population. Default is 100.
    ...
    c : float, optional
        Signal propaGWOtion speed (e.g., speed of light) in m/s. Default is AVG_LIGHT_SPEED.
    ...
    tuple
        ...
        sol_best_fitness : np.ndarray  
            Best fitness value obtained by GWO for each cluster.
    ...
    solver = StelaGWO(epoch=max_epochs,
...
def runner_GWO_process(params):
    ...
    return runner_GWO(event_positions=event_positions,
                      event_times=event_times,
                      spatial_clusters=spatial_clusters,
                      sensor_tt=sensor_tt,
                      sensor_indexes=sensor_indexes,
