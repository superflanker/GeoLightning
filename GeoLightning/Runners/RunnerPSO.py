"""
StelaPSO Runner Wrapper
=======================

Summary
-------
Wrapper function for executing the StelaPSO algorithm in the context of
...
- Execution of the StelaPSO algorithm
...
def runner_PSO(event_positions: np.ndarray,
    ...
    """
    Executes the Particle Swarm Optimization (PSO) algorithm for estimating 
    the origin positions of events based on clustered detections and arrival time data.

    This function applies the PSO metaheuristic to solve the multilateration problem 
    ...
    max_epochs : int, optional
        Maximum number of iterations (epochs) for the PSO algorithm. Default is 100.

    max_population : int, optional
        Number of candidate solutions in the PSO population. Default is 100.
    ...
    tuple
        ...
        sol_best_fitness : np.ndarray  
            Best fitness value obtained by PSO for each cluster.
    ...
    """
    solver = StelaPSO(epoch=max_epochs,
...
def runner_PSO_process(params):
    ...
    return runner_PSO(event_positions=event_positions,
                      event_times=event_times,
                      spatial_clusters=spatial_clusters,
                      sensor_tt=sensor_tt,
                      sensor_indexes=sensor_indexes,
