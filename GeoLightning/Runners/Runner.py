...
Summary
-------
Wrapper function for executing the Metaheuristics algorithm in the context of
...
- Execution of the metaheuristics algorithm
...
def runner(solver: object,
    ...
    """
    Executes optimization algorithm for estimating 
    the origin positions of events based on clustered detections and arrival time data.

    This function applies the selected metaheuristic to solve the multilateration problem 
    ...
    ----------
    solver: object
        the selected metaheuristics solver

    seed: np.int32
        the selected fixed seed to the multilateration data generation 
    ...
    ordered_indexes = problem.ordered_indexes

    spatial_clusters = spatial_clusters[ordered_indexes]
