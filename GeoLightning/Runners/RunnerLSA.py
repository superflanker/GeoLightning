"""
StelaLSA Runner Wrapper
=======================

Summary
-------
Wrapper function for executing the StelaLSA algorithm in the context of
...
- Execution of the StelaLSA algorithm
...
def runner_LSA(event_positions: np.ndarray,
    ...
    """
    Executes the Lightning Search Algorithm (LSA) algorithm for estimating 
    the origin positions of events based on clustered detections and arrival time data.

    This function applies the LSA metaheuristic to solve the multilateration problem 
    ...
    max_epochs : int, optional
        Maximum number of iterations (epochs) for the LSA algorithm. Default is 100.

    max_population : int, optional
        Number of candidate solutions in the LSA population. Default is 100.
    ...
    tuple
        ...
        sol_best_fitness : np.ndarray  
            Best fitness value obtained by LSA for each cluster.
    ...
    """
    solver = StelaLSA(epoch=max_epochs,
...
def runner_LSA_process(params):
    ...
    return runner_LSA(event_positions=event_positions,
                      event_times=event_times,
                      spatial_clusters=spatial_clusters,
                      sensor_tt=sensor_tt,
                      sensor_indexes=sensor_indexes,
